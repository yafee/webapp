...
$(function(){
  ...
  addMoreBtnTitle = '',
  secondMenu,
  listPage = recPage = xinPage = dreamPage = 1, // 数据分页
  ...
  var addMoreBtnChange = {
    ...
    findMoreBtn: function(title) { return this.checkContainer(title).find('.more').children('a'); },
  ...
  tabTits.on('click', function(){
    ...
    switch ($.trim($(this).html())) {
      case frist_tab_name.award: //金蜂鸟奖
        alert('ok');
        displayController([tuijian, quanbu, dream], award, true);
        break;
      case frist_tab_name.RECOMMEND:     // 推荐
        displayController([quanbu, renqi, dream], tuijian, true);
      ...
      case frist_tab_name.PERFORM:     // 演出众筹
        ...
        if ($.trim(quanbuList.html()) == '') {
          // 默认调用“全部”接口  
          listPage = 1; queryDataList($(this).html(), quanbuList);     
          // 给演出众筹的二级菜单加上点击事件
          ...
          secondMenu.each(function(){
            $(this).on('click', function(){
              ...
              // 每个二级标签点击时都重置数据
              quanbuList.empty(); listPage = 1; queryDataList($(this).find('span').html(), quanbuList);
            })
          })
        }  
        kind.is(':hidden') && kind.show();     // 首次加载只隐藏kind不隐藏quanbu，解决图片轮播加载bug。
        displayController([tuijian, renqi, dream], quanbu, true);
        break;
      case frist_tab_name.STAR:     // 星动众筹
        renqi.find('.more').show();    
        // 初始化
        if (!renqi.find('.renqi').html()) { xinPage = 1; queryStarData(); }
        displayController([tuijian, quanbu, dream], renqi, true);
        break;
  ...
  $("#left-nav").delegate('li', 'click', function() {
    ...
    switch($.trim(title)){
      ...
      case left_tab_name.PAI:
        ...
        // 初始化排行榜
        if($.trim(paihangList.html()) == ''){ listPage = 1; queryDataList(frist_tab_name.LATEST, paihangList); }
  ...
  if (session.getItem('status') != 'online') {     // 未登录用户 
    if (ua.match(/MicroMessenger/i) == "micromessenger" 
      && (session.getItem('flag') == '' || !session.getItem('flag'))) {     // 在微信中打开且微信未授权登陆
