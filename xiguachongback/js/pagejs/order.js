...
$(function() {
  ...
  var startTimeStr = year+'-'+month+'-'+'01';
  var endTimeStr = year+'-'+month+'-'+((date.getDate()+1 < 10) ? ('0' + (date.getDate()+1)) : (date.getDate()+1 + ''));
  ...
  $("#my-start").val(startTimeStr);
  $("#my-end").val(endTimeStr);
  ...
  $.get('/DealOrder/query', {
    ...
    // init datatable
  new DataTable(
    $('.dataTable'), ['订单ID', '地址', '省份', '城市', '收件人', '收件人电话', '订单创建时间', '演出地址', '演出名称', '演出时间', '抵扣金钱', '订单号', '订单状态', '付款时间', '邮费', '运送方式', '总价', '用户昵称'],
    '/DealOrder/query',
    'GET', {
      beginTime: $('#my-start').val(),
      endTime: $('#my-end').val(),
  ...
  $('#start').on('click', function() {
    if (!$('#start').hasClass('am-disabled')) {
      ...
      pageBtnOff.start();
      page = 1;
      queryUserInfo($('#my-start').val(), $('#my-end').val(), null, null, null, null, page);
      renderPage(page);
      $('#page').html(page);
    }
  })
  ...
  $('#find').on('click', function() {
    ...
    if (selection == '订单号') {
      ...
      queryUserInfo($('#my-start').val(), $('#my-end').val(), null, keyWords, null, null, 1);
    } else if (selection == '用户昵称') {
      queryUserInfo($('#my-start').val(), $('#my-end').val(), null, null, keyWords, null, 1);
    } else if (selection == '项目名称') {
      queryUserInfo($('#my-start').val(), $('#my-end').val(), null, null, null, keyWords, 1);
    }
    // 用户数据只有一条，所以写死只有一页
    var pages = '';
    $('.page').remove();
  ...
  function queryUserInfo(beginTime, endTime, orderNo, mobile, userName, dealName, pageIndex) {
    ...
    $.get('/DealOrder/query', {
      ...
      $('.page').remove();
      $('#start').after(pages);
      $('.page:eq('+(pageIndex-1)+')').addClass('am-active');
      $('#total').html(totalPage);
      $('#page').html(page);
      $('#totalNumber').html(data.totalNumber);
      $('#totalDiscount').html('¥' + data.returnData.orderSum.totalDiscount);
