 ...
 */      
var DataTable = function(tablediv, titles, url, method, userdata, datakey, columns, delurl, delparams, updateurl, addurl, cb){
	var _this = this;      
	var tableEle; 
		...
		if (data[0] instanceof Array) {     // data是二维数组（集合）
			for (i in data) {
				...
				tableEle.find('tbody').append(     // 初始化数据 
					'<tr>' + _this.tds + '<td><button style="display: none;">增</button><button>删</button><button>改</button></td></tr>'              
				)  
	...
	DataTable.prototype.delData = function(){          
		if(arguments.length == 0){     // 原有的“删”按钮进行删除行操作 
			...
			// 后台删除
			del(arguments[0], delparams);
		} 
		
		function del(ele, delparams) {
			// var parameters = '',
			// 	i = 0,
			// 	len = columns.length;
			// for ( ; i < len; i++) {
