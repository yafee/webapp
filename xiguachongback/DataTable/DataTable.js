 ...
 */                       
var DataTable = function(tablediv, titles, url, method, userdata, columns){
	var _this = this;
	// 定义表格和所有按钮
	_this.tableEle;
	_this.addBtns;
	...
	DataTable.prototype.init = function(){
		...
		tablediv.append(
			...
			'<table cellpadding="0" cellspacing="0"><thead><tr>' + tits + '<td>操作</td></tr></thead><tbody></tbody></table>'
		)
		// 请求数据
		...
		_this.tableEle = tablediv.find('table');     // 初始化table节点         
	}        
	_this.init();     // 必须先初始化表格！！！ 
	...
	function parseData(data) { 
		if (data) {      
			...
			if (data.length > 1) {     // 拿到的是集合 
				...
				data = data.substring(1, data.length-1);     // 去大括号   
				data = data.replace(/\"/g, '');     // 去双引号
				data = data.split('},{');     // 去“},{”，将每条数据抽出 
	...
	function initData(data) {
		...
		if (data[0] instanceof Array) {     // 二维
			for (i in data) {
				...
				_this.tableEle.find('tbody').append(     // 初始化数据 
					'<tr>' + tds + '<td><button>增</button><button>删</button><button>改</button></td></tr>'              
				) 
				tds = '';     // 每条数据添加完毕后需重置tds
			}
		} else {     // 一维
			...
			_this.tableEle.find('tbody').append( 
				'<tr>' + tds + '<td><button>增</button><button>删</button><button>改</button></td></tr>' 
			)
		}
		
		// 初始化按钮
		_this.addBtns = _this.tableEle.find('button:contains("增")');     // 初始化时的所有“增”按钮（不含新增的按钮）
		_this.delBtns = _this.tableEle.find('button:contains("删")');     // ...“删”...
		_this.modBtns = _this.tableEle.find('button:contains("改")');     // ...“改”...
